...
class SmoothGroupApp:
    def __init__(self):
        ...
        self.screen_status_var = tk.StringVar(value="[截图] 未加载")
        ...
        tk.Button(control_frame, text="加载手机截图", command=self.load_screenshot).pack(side="left")
        tk.Button(control_frame, text="刷新截图", command=self.refresh_screenshot).pack(side="left", padx=6)
        tk.Button(control_frame, text="清空截图", command=self.clear_screenshot).pack(side="left")

        self.canvas = tk.Canvas(
            self.root,
            width=int(REAL_W * DISPLAY_SCALE),
            height=int(REAL_H * DISPLAY_SCALE),
            bg="#1a1a1a",
        )
    ...
    def load_screenshot(self):
        if Image is None or ImageTk is None:
            self.screen_status_var.set("[截图] 未安装 Pillow，无法显示图片")
            return
        ...
        self._draw_screenshot(file_path)

    def refresh_screenshot(self):
        if not self.screenshot_path:
            self.screen_status_var.set("[截图] 还没有加载截图")
            return
        self._draw_screenshot(self.screenshot_path)

    def clear_screenshot(self):
        if self.screen_item_id is not None:
            self.canvas.delete(self.screen_item_id)
            self.screen_item_id = None
        self.screen_image_ref = None
        self.screen_status_var.set("[截图] 已清空")

    def _draw_screenshot(self, image_path):
        try:
            image = Image.open(image_path).convert("RGB")
            image = image.resize((int(REAL_W * DISPLAY_SCALE), int(REAL_H * DISPLAY_SCALE)))
            tk_img = ImageTk.PhotoImage(image)
        except Exception as e:
            self.screen_status_var.set(f"[截图] 加载失败: {e}")
            return
        ...
        self.screen_status_var.set(f"[截图] 已加载: {image_path}")
